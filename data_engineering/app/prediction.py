...
def generate_dummy_data():
    ...
    occupation = np.random.choice(['Housemaid', 'Services', 'Admin', 'Blue-collar', 'Technician',
                                         'Retired', 'Management', 'Unemployed', 'Self-employed', 'Entrepreneur', 'Student'])
    marital_status = np.random.choice(['Married', 'Single', 'Divorced'])
    education_level = np.random.choice(['Basic (4 Years)', 'High School', 'Basic (6 Years)', 'Basic (9 Years)',
                                              'Professional Course', 'University Degree', 'Illiterate'])
    credit_default = np.random.choice(['No', 'Yes'])
    housing_loan = np.random.choice(['No', 'Yes'])
    personal_loan = np.random.choice(['No', 'Yes'])
    contact_method = np.random.choice(['Telephone', 'Cellular'])
    last_contact_month = np.random.randint(1, 12)
    last_contact_day_of_week = np.random.choice(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'])
    last_contact_duration = np.random.randint(1, 5000)
    campaign_contacts = np.random.randint(1, 100)
    days_since_previous_contact = np.random.randint(1, 999)
    previous_contacts = np.random.randint(1, 10)
    previous_campaign_outcome = np.random.choice(['Non-existent', 'Failure', 'Success'])
    employment_variation_rate = np.random.uniform(-5.0, 5.0)
    consumer_price_index = np.random.uniform(-100.0, 100.0)
    consumer_confidence_index = np.random.uniform(-50.0, 100.0)
    euribor_3_month_rate = np.random.uniform(0.000, 9.999)
    number_employed = np.random.randint(1000, 99999)
...
def run():
   # Set title
    st.title("Model Inference for Predicting.")

    # Sub title
    st.subheader("Predicting the Possibility Customer Will Subscribe Or Not.")
    st.markdown('---')
    st.image("https://img.harianjogja.com/posts/2024/01/07/1160717/bank-freepik.jpg")
    ...
    with st.form('my_form'):
        ...
        previous_contacts = st.number_input(label='Enter previous contacts', min_value = 0, max_value= 10, step=1)
        ...
        # Create a button
        submit = st.form_submit_button('Collect Data')
